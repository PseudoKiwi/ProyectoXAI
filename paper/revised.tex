...
We compare attention maps with complementary geometric, probabilistic, and spectral measures. Given two attention matrices $A,B$ and their vectorizations $\vec A, \vec B$, we compared them via \textit{cosine similarity} $\cos(\theta)=\langle \vec A,\vec B\rangle/(\|\vec A\|_2\|\vec B\|_2)\in[-1,1]$, the \textit{Frobenius distance} $d_F(A,B)=\|A-B\|_F=\sum_{i,j}(A-B)^2_{ij}$ and the row-averaged \textit{Jensen--Shannon divergence} $D_{JS}(A||B)=\frac{1}{n}\sum_i D_{JS}(A_i||B_i)$ were $A_i, B_i$ are the $i_{th}$ rows of $A,B$ and $D_{JS}(A_i||B_i) = \frac{1}{2}D_{KL}(A_i||M_i) + \frac{1}{2}D_{KL}(B_i||M_i)$ with $M_i = (A_i + B_i)/2$. For $n \times n$ attention maps, the \textit{Frobenius distance} is bounded by $\sqrt{2n}$ ($64$ for our $n=2048$ maps). $D_{JS}$ is theoretically bounded in $[0,\ln 2]$ if measured in nats \cite{lin1991divergence}, where $0$ means equal distributions. For spectral metrics we propose the entropy-based \textit{effective rank} (ER) \cite{roy2007effective} and the \textit{Participation Ratio} (PR) \cite{recanatesi2022scale}.
...
\caption{\textit{Cosine similarity}, \textit{Frobenius distance}, row-averaged \textit{Jensen--Shannon divergence} ($D_{JS}$), Participation Ratio ($PR$), and Entropy Rank ($ER$) between each model's attention maps and the teacher's $P_{avg}$, both averaged over the test set (following $P_{avg_i} = (P_{2i}+P_{2i+1})/2$ for layer $i$ from teacher model $P$), for the non-distilled baseline ($B$) and the distilled students ($S_0$, $S_1$, $S_2$), on both datasets \textbf{IMDb} and \textbf{AG News}.}
