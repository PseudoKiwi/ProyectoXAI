...
\begin{figure}[bp]
\centering
\includesvg[width=\textwidth]{../images/architecture_design.svg}
\caption{}
...
The teacher is fully fine-tuned per dataset for one epoch (learning rate $2\times10^{-5}$, AdamW, early stopping after $5$ non-improving validation rounds). Every student is randomly initialized and trained for $3$ epochs (learning rate $1\times10^{-4}$, no early stopping), minimizing \eqref{eq:loss}.
...
We compare Attention maps with complementary geometric, probabilistic, and spectral measures. Given two attention matrices $A,B$ and their vectorizations $\vec A, \vec B$, we compared them via \textit{cosine similarity} $\cos(\theta)=\langle \vec A,\vec B\rangle/(\|\vec A\|_2\|\vec B\|_2)\in[-1,1]$, the \textit{Frobenius distance} $d_F(A,B)=\|A-B\|_F=\sum_{i,j}(A-B)^2_{ij}$ and the row-averaged \textit{Jensen--Shannon divergence} $\frac{1}{n}\sum_i JS(A_i||B_i)$ were $A_i$ is the $i_{th}$ row of $A$. For $n \times n$ Attention maps, the \textit{Frobenius distance} is bounded by $\sqrt{2n}$ ($64$ for our $n=2048$ maps). The \textit{Jensen--Shannon divergence} is theoretically bounded in $[0,\ln 2]$ if measured in nats \cite{lin1991divergence}, where $0$ means equal distributions. For spectral metrics we propose the entropy-based \textit{effective rank} \cite{roy2007effective} and the \textit{Participation Ratio} \cite{recanatesi2022scale}.
...
\begin{table}[bp]
\centering
\caption{\textbf{LIME} stability (mean pairwise Jaccard@10 across $5$ seeds, $N=20$), both datasets. Bold: best model per row; underlined: second-best.}
\small
\begin{tabular}{|l|c|c|c|c|c|}
\hline
Dataset & $P$ & $B$ & $S_0$ & $S_1$ & $S_2$ \\ \hline
\textbf{IMDb} & $0.548$ & $0.587$ & $\underline{0.684}$ & $0.611$ & $\mathbf{0.710}$ \\
\textbf{AG News} & $\mathbf{0.773}$ & $0.668$ & $0.638$ & $0.614$ & $\underline{0.688}$ \\ \hline
\end{tabular}
\label{tab:stability}
\end{table}
...
\textbf{IMDb} & $P$ & -- & -- & $0.172$ & -- & -- & $\underline{0.118}$ & -- & -- & $0.010$ \\
\textbf{IMDb} & $B$ & $\underline{0.183}$ & $\underline{0.248}$ & $0.083$ & $0.117$ & $0.239$ & $0.025$ & $\underline{0.793}$ & $-0.005$ & $0.010$ \\
\textbf{IMDb} & $S_0$ & $0.170$ & $0.216$ & $\underline{0.202}$ & $\underline{0.123}$ & $0.238$ & $0.093$ & $0.778$ & $\underline{0.028}$ & $\mathbf{0.085}$ \\
\textbf{IMDb} & $S_1$ & $0.136$ & $0.189$ & $0.177$ & $0.096$ & $\underline{0.245}$ & $0.058$ & $\mathbf{0.814}$ & $0.010$ & $\underline{0.025}$ \\
\textbf{IMDb} & $S_2$ & $\mathbf{0.212}$ & $\mathbf{0.299}$ & $\mathbf{0.267}$ & $\mathbf{0.176}$ & $\mathbf{0.333}$ & $\mathbf{0.120}$ & $0.787$ & $\mathbf{0.039}$ & $0.019$ \\
\hline
\textbf{AG News} & $P$ & -- & -- & $0.138$ & -- & -- & $0.002$ & -- & -- & $0.053$ \\
\textbf{AG News} & $B$ & $0.336$ & $\underline{0.321}$ & $0.306$ & $0.315$ & $\underline{0.269}$ & $-0.002$ & $\mathbf{0.890}$ & $\underline{0.021}$ & $0.067$ \\
\textbf{AG News} & $S_0$ & $\underline{0.347}$ & $0.279$ & $\underline{0.402}$ & $\underline{0.317}$ & $\mathbf{0.274}$ & $0.000$ & $0.883$ & $\mathbf{0.040}$ & $\underline{0.162}$ \\
\textbf{AG News} & $S_1$ & $0.341$ & $0.306$ & $0.299$ & $\mathbf{0.320}$ & $0.267$ & $0.000$ & $0.876$ & $-0.036$ & $0.138$ \\
\textbf{AG News} & $S_2$ & $\mathbf{0.358}$ & $\mathbf{0.342}$ & $\mathbf{0.431}$ & $0.311$ & $0.262$ & $-0.000$ & $\underline{0.888}$ & $0.013$ & $\mathbf{0.340}$ \\ \hline
\end{tabular}
\label{tab:explanation_metrics}
\end{table*}

By contrast, Frobenius distance and $JS$ divergence (Fig.~\ref{fig:teacher_avg_comparison_grid}) are nearly indistinguishable across all four models at every layer on both datasets, following the same profile: near-perfect alignment at $i=0$, peaking at $i=2$--$3$ (Frobenius $34$--$41$ out of a theoretical maximum of $64$; $JS$ $0.34$--$0.50$), then stabilizing by $i=4$--$10$. At an even finer grain, per-head attention-similarity analysis shows $B$ numerically indistinguishable from the three distilled students on \emph{both} datasets -- the clearest evidence that per-head similarity to the teacher is insensitive to whether a model was distilled.

Fig.~\ref{fig:teacher_avg_comparison_grid} also reports Participation Ratio ($PR$) and entropy-based effective rank ($ER$) on both datasets. On both measures, $P_{avg}$ stays close to rank-one throughout, while the four students show no consistent ranking: on \textbf{AG News}, $B$ trends from lowest to highest as depth increases on both $PR$ and $ER$; on \textbf{IMDb}, the relative order among $B$, $S_0$, $S_1$, and $S_2$ instead shifts from layer to layer. Spectral complexity is therefore not obviously ordered by distillation strength on either dataset.
...
The authors extend their sincere gratitude to Dr. Mauricio Velasco for his valuable feedback. The writing of this manuscript was partially assisted by generative language models used exclusively for stylistic revision and textual reformulation tasks: Claude Sonnet 4.6 \cite{anthropic2025claude} (Anthropic).
